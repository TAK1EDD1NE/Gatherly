...
export default function Sidebar4({ ...props }) {
  ...
  return (
    <Sidebar
      ...
      collapsed={collapsed}
      rootStyles={{ [`.\${sidebarClasses.container}`]: { gap: 28 } }} // Corrected gap to 28
      className={`${props.className} flex flex-col self-center h-screen pt-5 gap-7 top-0 bg-[#ffffff] !sticky overflow-auto`} // Corrected className
    >
      <div className="flex self-stretch px-5 mx-11">
        <Img src="images/img_sidebar_logo.png" alt="Sidebarlogo" className="h-[30px] w-[102px] object-contain" /> {/* Corrected width to 102px */}
      </div>
      <Menu
        menuItemStyles={{
          button: {
            ...
            fontWeight: 600,
            fontSize: "14px",
            [`&:hover, &.ps-active`]: { color: "#ffffff", backgroundColor: "#f361e9 !important" },
          },
        }}
      ...
      >
        ...
        <div className="mt-[30px] h-px bg-[#e0e0e0]" />
      </Menu>
    </Sidebar>
  );
}